"""
A simple feed-forwarded network in chapter 2.
"""

import numpy as np

graph = []
layers = [8,40,20,1] # number of neurons per layer
input_num = layers[0]
output_num = layers[-1]
...
graph_datas = f"graph_dense_I{input_num}_O{output_num}_L{L}_H{H}.dat"
f = open(graph_datas,"w")
for line in to_file_graph:
    f.write(line+"\n")
f.close()

logic_datas = f"logic_dense_I{input_num}_O{output_num}_L{L}_H{H}.dat"
f = open(logic_datas,"w")
for line in to_file_logic:
    f.write(line+"\n")
f.close()

fixwb_datas = f"fixwb_dense_I{input_num}_O{output_num}_L{L}_H{H}.dat"
f = open(fixwb_datas,"w")
for line in to_file_fixwb:
    f.write(line+"\n")
f.close()

print(f"Id: *dense_I{input_num}_O{output_num}_L{L}_H{H}.dat")
